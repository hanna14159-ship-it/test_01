import sqlite3from copy import copyfrom datetime import datetimefrom io import BytesIOfrom pathlib import Pathfrom zoneinfo import ZoneInfo

import streamlit as stfrom openpyxl import load_workbookfrom openpyxl.styles import Alignment
...
DB_PATH = "steamer.db"TEMPLATE_PATH = "steamer_template.xlsx"

KST = ZoneInfo("Asia/Seoul")

MACHINES = ["11", "12", "13", "14","21", "22", "23", "24","51", "52", "53", "54",]
...
MACHINE_ROW_MAP = {"11": 8,"12": 9,"13": 10,"14": 11,"21": 12,"22": 13,"23": 14,"24": 15,"51": 16,"52": 17,"53": 18,"54": 19,}
...
def create_table():"""점검 기록 테이블 생성.기존 DB에 새 칼럼이 없으면 자동 추가."""
...
def save_or_update_record(record):"""같은 날짜 + 같은 호기 기록이 없으면 새로 저장.이미 있으면 기존 기록 수정."""
...
def load_records(selected_date):"""선택한 날짜의 점검 기록을 불러온다."""
...
def delete_record(record_id):"""id 기준으로 점검 기록 한 건 삭제."""
...
def parse_number(value, label):"""문자 입력값을 숫자로 변환한다.
...
def format_number(value):"""Excel 한 칸 안에 들어갈 숫자를 보기 좋게 변환.None이면 빈칸."""
...
def number_text_input(label, key):"""+ / - 버튼 없는 숫자 입력칸."""
...
def get_machine_from_url():"""URL의 machine 값을 읽는다.
...
def make_flow_pressure_text(flow, pressure):"""유량과 압력을 한 셀에 두 줄로 넣는다.
...
def write_multiline_cell(worksheet, cell_address, value):"""원본 셀 서식은 건드리지 않고 값만 넣는다.줄바꿈 표시를 위해 wrap_text만 기존 정렬 기반으로 켠다.
...
def make_template_excel(selected_date):"""기존 .xlsx 원본 양식을 그대로 열어서값만 지정된 셀에 입력한다.
...
st.set_page_config(page_title="증숙기 점검일지",page_icon="📝",layout="wide",)

create_table()

qr_machine = get_machine_from_url()

if qr_machine is not None:default_machine_index = MACHINES.index(qr_machine)else:default_machine_index = 0
...
tab_input, tab_records, tab_manage, tab_excel = st.tabs(["점검 입력","저장 기록","기록 관리","Excel 다운로드",])
